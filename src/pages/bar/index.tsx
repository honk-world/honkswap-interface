...
import { HONK, XHONK } from '../../config/tokens'
...
export default function Stake() {
  ...
  const honkBalance = useTokenBalance(account ?? undefined, HONK[chainId])
  const xHonkBalance = useTokenBalance(account ?? undefined, XHONK[chainId])

  const honkPrice = useHonkPrice()

  const { enter, leave } = useSushiBar()

  //const { data } = useSWR(`{bar(id: "0xbE30834C394a8db13F38487d4A71f6AC26859b05") {ratio, totalSupply}}`, fetcher) //todo: update with honkbar
  ...
  const balance = activeTab === 0 ? honkBalance : xHonkBalance
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const apr = (((results[1].volumeUSD * 0.05) / data?.bar?.totalSupply) * 365) / (data?.bar?.ratio * honkPrice)
      setApr(apr)
    }
    fetchData()
  }, [data?.bar?.ratio, data?.bar?.totalSupply, honkPrice])
    ...
    <Container id="bar-page" className="py-4 md:py-8 lg:py-12" maxWidth="full">
      <Head>
        ...
        <meta
          key="description"
          name="description"
          content="Stake HONK in return for xHONK, an interest bearing and fungible SEP20 token designed to share revenue generated by all HONK products."
        />
        <meta key="twitter:url" name="twitter:url" content="https://app.honkswap.fi/stake" />
        <meta key="twitter:title" name="twitter:title" content="STAKE HONK" />
        <meta
          key="twitter:description"
          name="twitter:description"
          content="Stake HONK in return for xHONK, an interest bearing and fungible SEP20 token designed to share revenue generated by all HONK products."
        />
        <meta key="twitter:image" name="twitter:image" content="https://app.honkswap.fi/xhonk-sign.png" />
        <meta key="og:title" property="og:title" content="STAKE HONK" />
        <meta key="og:url" property="og:url" content="https://app.honkswap.fi/stake" />
        <meta key="og:image" property="og:image" content="https://app.honkswap.fi/xhonk-sign.png" />
        <meta
          key="og:description"
          ...
          <div className="flex flex-col w-full max-w-xl mt-auto mb-2">
            <div className="flex max-w-lg">
              <div className="self-end mb-3 text-lg font-bold md:text-2xl text-high-emphesis md:mb-7">
                {i18n._(t`Maximize yield by staking HONK for xHONK`)}
              </div>
              {/* <div className="self-start pl-6 pr-3 mb-1 min-w-max md:hidden">
                                <img src={XSushiSignSmall} alt="xhonk sign" />
                            </div> */}
            </div>
            ...
            {/* <div className="flex">
                            ...
                            <div>
                                <StyledLink className="text-sm text-lg whitespace-nowrap md:text-lg md:leading-5">
                                    Tips for using xHONK
                                </StyledLink>
                            </div>
                        </div> */}
          </div>
          <div className="hidden px-8 ml-6 md:block w-72">
            <Image src="/xhonk-sign.png" alt="xHONK sign" width="100%" height="100%" layout="responsive" />
          </div>
        </div>
            ...
            <div className="mb-4">
              {
                <div className="flex items-center justify-between w-full h-24 max-w-xl p-4 rounded md:pl-5 md:pr-7 bg-light-yellow bg-opacity-40">
                  <div className="flex flex-col">
                    ...
                    <div className="flex">
                      <a
                        href={`https://analytics.honkswap.fi/bar`}
                        target="_blank"
              ...
              <div className="w-full max-w-xl px-3 pt-2 pb-6 rounded bg-dark-900 md:pb-9 md:pt-4 md:px-8">
                ...
                <div className="flex items-center justify-between w-full mt-6">
                  <p className="font-bold text-large md:text-2xl text-high-emphesis">
                    {activeTab === 0 ? i18n._(t`Stake HONK`) : i18n._(t`Unstake`)}
                  </p>
                  <div className="border-gradient-r-pink-red-light-brown-dark-pink-red border-transparent border-solid border rounded-3xl px-4 md:px-3.5 py-1.5 md:py-0.5 text-high-emphesis text-xs font-medium md:text-base md:font-normal">
                    {`1 xHONK = ${xSushiPerSushi.toFixed(4)} HONK`}
                  </div>
                </div>
                ...
                <div className="relative w-full h-0 pointer-events-none bottom-14">
                  ...
                  >
                    <div className="flex space-x-2 ">
                      ...
                      >
                        {`${input ? input : '0'} ${activeTab === 0 ? '' : 'x'}HONK`}
                      </p>
                    </div>
              ...
              <div className="flex flex-wrap">
                <div className="flex flex-col flex-grow md:mb-14">
                  ...
                  <div className="flex items-center space-x-4">
                    <Image
                      ...
                      src="/images/tokens/xhonk-square.png"
                      alt="xHONK"
                      width={64}
                      height={64}
                    />
                ...
                <div className="flex flex-col flex-grow">
                  ...
                  <div className="flex items-center ml-8 space-x-4 md:ml-0">
                    <Image
                      className="max-w-10 md:max-w-16 -ml-1 mr-1 md:mr-2 -mb-1.5 rounded"
                      src="/images/tokens/honk-square.png"
                      alt="HONK"
                      width={64}
                      height={64}
                    />
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {honkBalance ? honkBalance.toSignificant(8) : '-'}
                      </p>
                      <p className="text-sm md:text-base text-primary">HONK</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col w-full mb-4 mt-7 md:mb-0">
                  {account && (
                    <a
                      href={`https://analytics.honkswap.fi/users/${account}`}
                      target="_blank"
                      rel="noreferrer noopener"
                      className={`
